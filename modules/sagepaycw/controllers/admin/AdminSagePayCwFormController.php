 ...
 *       		   	    	 		 
 */
class AdminSagePayCwFormController extends AdminController
{
	public function __construct() {
		$this->className = 'AdminSagePayCwFormController';
		parent::__construct();
		$this->context->smarty->addTemplateDir($this->getTemplatePath());
		$this->tpl_folder = 'sagepaycw_form/';
		$this->bootstrap = true;
	}
...
require_once 'SagePayCw/BackendFormRenderer.php';
	...
	private function createCronJobUrl() {
		$link = new Link();
		return $link->getModuleLink('sagepaycw', 'cron', array(), true);
	}
	...
	private function handleView() {
		...
		if ($form->isProcessable()) {
			$url = SagePayCw::getAdminUrl('AdminSagePayCwForm', array('save' => 'true', 'form' => $form->getMachineName()));
	...
	private function getBackendFormAdapter() {
		return SagePayCw_Util::getBackendFormAdapter();
	}

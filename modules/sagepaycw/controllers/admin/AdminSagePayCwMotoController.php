 ...
 *       		   	    	 		 
 */
class AdminSagePayCwMotoController extends AdminController
{
	public function __construct() {
		$this->className = 'AdminSagePayCwMotoController';
		parent::__construct();
		$this->context->smarty->addTemplateDir($this->getTemplatePath());
		$this->tpl_folder = 'sagepaycw_moto/';
		$this->bootstrap = true;
	}
	...
	public function initContent()
	{
		...
		if (isset($_GET['cw_transaction_id'])) {
			...
			$dbTransaction = SagePayCw_Entity_Transaction::loadById($_GET['cw_transaction_id']);
			if ($dbTransaction->getTransactionObject()->isAuthorized()) {
				...
				$params['vieworder'] = '1';
				header('Location: ' . SagePayCw::getAdminUrl('AdminOrders', $params));
				die();
			}
			else {
		...
		/* @var $moduleInstance SagePayCw_IPaymentMethod */
		...
		$adapter = SagePayCw_Util::getAuthorizationAdapter(Customweb_Payment_Authorization_Moto_IAdapter::AUTHORIZATION_METHOD_NAME);
		...
		if ($isSupported) {
			...
			if ($visibleFormFields !== null && count($visibleFormFields) > 0) {
				$renderer = new SagePayCw_FormRenderer($moduleInstance->getPaymentMethodName());

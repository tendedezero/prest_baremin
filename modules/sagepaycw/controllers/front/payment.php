 ...
 *
 */
class SagePayCwPaymentModuleFrontController extends ModuleFrontController {
	public $ssl = true;
...
require_once 'SagePayCw/Entity/Transaction.php';
		...
		$this->addJS(_MODULE_DIR_ . 'sagepaycw/js/frontend.js');
		...
		if ($errorTransactionId !== null) {
			$errorDbTransaction = SagePayCw_Entity_Transaction::loadById(intval($errorTransactionId));
		...
		try {
			$adapter = SagePayCw_Util::getShopAdapterByPaymentAdapter($module->getAuthorizationAdapter($module->getOrderContext()));

			$createTransaction = true;
			$renderOnloadJs = true;
			if (isset($_REQUEST['ajaxAliasForm']) || SagePayCw::getInstance()->isCreationOfPendingOrderActive()) {

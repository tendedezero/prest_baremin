 ...
 *
 */
class SagePayCwTimeoutModuleFrontController extends ModuleFrontController {
	public $ssl = true;
	...
	private function getTimeoutErrorMessage(){
		$error = $this->getTranslator()->trans(
				...
				array(), 'Module.sagepaycw');

		$transactionId = Tools::getValue('cw_transaction_id');
		if ($transactionId !== null) {
			$transaction = SagePayCw_Entity_Transaction::loadById($transactionId);
			if ($transaction !== null) {
				$error .= sprintf($this->getTranslator()->trans(' Please mention the following transaction id: %s.', array(), 'Module.sagepaycw'),

 ...
 *
 */
class SagePayCw_Adapter_WidgetAdapter extends SagePayCw_Adapter_AbstractAdapter {
	...
	public function prepareWithFormData(array $formData, SagePayCw_Entity_Transaction $transaction) {
		...
		if ($transaction->getTransactionObject()->isAuthorizationFailed()) {
			...
			if (is_object($errorMessage)) {
				...
				$this->errorMessage = SagePayCw::translate("Failed to initialize transaction with an unknown error");
			}
		}
	}
	...
	private function createFormUrl() {
		...
		return $link->getModuleLink('sagepaycw', 'widget', $parameters, true);
	}

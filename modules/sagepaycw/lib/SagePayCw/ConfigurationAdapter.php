 ...
 * @Bean
 */
class SagePayCw_ConfigurationAdapter implements Customweb_Payment_IConfigurationAdapter{
	...
	public function existsConfiguration($key, $language = null) {
		...
		if ($language !== null) {
			...
			$langId = SagePayCw_Util::getLanguageIdByIETFTag($language);
		}
	...
	public function getStoreHierarchy() {
		...
		if (!empty(Context::getContext()->cookie->shopContext)) {
			if (strpos(Context::getContext()->cookie->shopContext, 'g-') === 0) {
				...
				return array(
					'default' => SagePayCw::translate('Default'),

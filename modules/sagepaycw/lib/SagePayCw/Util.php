...
require_once 'SagePayCw/DatabaseLinkAccessor.php';
...
final class SagePayCw_Util {
	...
	public static function getCheckoutCookieKey(Customweb_Payment_Authorization_IPaymentMethod $paymentMethod){
		return 'sagepaycw_checkout_id' . $paymentMethod->getPaymentMethodName();
	}
	...
	public static function createContainer(){
		if (self::$container === null) {
			...
			$packages[] = 'SagePayCw_LayoutRenderer';
			$packages[] = 'SagePayCw_ExternalCheckoutService';
			$packages[] = 'SagePayCw_EndpointAdapter';
			...
			$storageBackend = new Customweb_Storage_Backend_Database(self::getEntityManager(), self::getDriver(), 'SagePayCw_Entity_Storage');
	...
	public static function getEndpointDispatcher(){
		return new Customweb_Payment_Endpoint_Dispatcher(self::createContainer()->getBean('SagePayCw_EndpointAdapter'),
	...
	public static function getDriver(){
		if (self::$driver === null) {
			...
			$link = SagePayCw_DatabaseLinkAccessor::getSagePayCwLink($databaseInstance);
	 ...
	 * @throws Exception
	 * @return SagePayCw_Adapter_IAdapter
	 */
	public static function getShopAdapterByPaymentAdapter(Customweb_Payment_Authorization_IAdapter $paymentAdapter){
		...
		$adapters = self::createContainer()->getBeansByType('SagePayCw_Adapter_IAdapter');
		foreach ($adapters as $adapter) {
			if ($adapter instanceof SagePayCw_Adapter_IAdapter) {
	...
	public static function getAssetResolver(){
		...
		if (!isset(self::$resolvers[$shopId])) {
			...
			$moduleUrl = $baseUrl . 'modules/sagepaycw';

			$currentTemplatePath = _PS_ALL_THEMES_DIR_ . $context->shop->getTheme() . '/modules/sagepaycw';
			$currentTemplateUrl = $baseUrl . '/themes/' . $context->shop->getTheme() . '/modules/sagepaycw';
	...
	public static function extractMailVariables(Customweb_Payment_Authorization_ITransaction $transaction){
		...
		$variables['{sagepaycw_transaction_id}'] = $transaction->getTransactionId();
		$variables['{sagepaycw_payment_id}'] = $transaction->getPaymentId();

		$variables['transaction_id'] = $transaction->getPaymentId();

		foreach ($transaction->getTransactionLabels() as $labelKey => $label) {
			$variables['{sagepaycw_label_' . $labelKey . '}'] = $label['value'];
		}

		if ($transaction->getPaymentInformation() !== null) {
			$variables['{sagepaycwpayment_information}'] = $transaction->getPaymentInformation();
			$variables['{sagepaycw_payment_information}'] = $transaction->getPaymentInformation();
			$variables['{sagepaycw_payment_information_txt}'] = Customweb_Core_Util_Html::toText($transaction->getPaymentInformation());
		}
	...
	public static function getOrderCreationMessage($id_employee){
		...
		if (isset($id_employee) && $id_employee > 0) {
			...
			$message = SagePayCw::translate('Manual order -- Employee:');
	...
	public static function createLineItemsFromCart(Cart $cart, $orderTotal, Customweb_Payment_Authorization_IPaymentMethod $paymentMethod = null){
		...
		if ($wrappingCosts > 0) {
			...
			$items[] = new Customweb_Payment_Authorization_DefaultInvoiceItem('wrapping', SagePayCw::translate("Wrapping Fee"), $taxRate,
		...
		if (method_exists($cart, 'getFee') && $paymentMethod != null) {
			$cart->getFee('sagepaycw_' . $paymentMethod->getPaymentMethodName());
			$feeamount = $cart->feeamount;
			if ($feeamount != 0) {
				$items[] = new Customweb_Payment_Authorization_DefaultInvoiceItem('payment-fee', SagePayCw::translate("Payment Fee"), 0,

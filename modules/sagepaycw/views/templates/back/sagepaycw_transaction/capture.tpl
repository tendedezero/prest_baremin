...
{if $transaction->getTransactionObject()->isPartialCapturePossible()}
	<form method="POST" class="sagepaycw-line-item-grid" id="capture-form">
		...
		<table class="table table-striped table-condensed table-hover table-bordered">
			...
			<tfoot>
				<tr>
					<td colspan="6" class="text-right">{lcw  mod='sagepaycw' s='Total Capture Amount'}:</td>
		...
		{if $transaction->getTransactionObject()->isCaptureClosable()}
			<div class="closable-box">
				<label for="close-transaction">{lcw  mod='sagepaycw' s='Close transaction for further captures'}</label>
				<input id="close-transaction" type="checkbox" name="close" value="on" />
			</div>
		{/if}
		
		<div class="text-right">
			<input type="submit" class="button btn btn-success" value="{lcw  mod='sagepaycw' s='Capture'}" />
		</div>
	</form>
{/if}

...
class AngarSlider extends Module implements WidgetInterface
{
    protected $_html = '';
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->getTranslator()->trans('AT - Image slider', array(), 'Modules.Imageslider.Admin');
        $this->description = $this->getTranslator()->trans('Adds an image slider to your site.', array(), 'Modules.Imageslider.Admin');
        $this->ps_versions_compliancy = array('min' => '1.7.0.0', 'max' => _PS_VERSION_);

        $this->templateFile = 'module:angarslider/views/templates/hook/slider.tpl';
    }
    ...
    public function uninstall()
    {
        ...
        if (parent::uninstall()) {
            ...
            $res &= Configuration::deleteByName('ANGARSLIDER_PAUSE_ON_HOVER');
            $res &= Configuration::deleteByName('ANGARSLIDER_PAGER');
    ...
    protected function _postValidation()
    {
        ...
        if (Tools::isSubmit('submitSlider')) {
            if (!Validate::isInt(Tools::getValue('ANGARSLIDER_PAUSE'))) {
    ...
    protected function _postProcess()
    {
        ...
        if (Tools::isSubmit('submitSlider')) {
            ...
            foreach ($shops as $shop_id) {
                ...
                $res &= Configuration::updateValue('ANGARSLIDER_PAUSE_ON_HOVER', (int)Tools::getValue('ANGARSLIDER_PAUSE_ON_HOVER'), false, $shop_group_id, $shop_id);
                $res &= Configuration::updateValue('ANGARSLIDER_PAGER', (int)Tools::getValue('ANGARSLIDER_PAGER'), false, $shop_group_id, $shop_id);
            }
            ...
            switch ($shop_context) {
                case Shop::CONTEXT_ALL:
                    ...
                    $res &= Configuration::updateValue('ANGARSLIDER_PAUSE_ON_HOVER', (int)Tools::getValue('ANGARSLIDER_PAUSE_ON_HOVER'));
                    $res &= Configuration::updateValue('ANGARSLIDER_PAGER', (int)Tools::getValue('ANGARSLIDER_PAGER'));
                    if (count($shop_groups_list)) {
                        foreach ($shop_groups_list as $shop_group_id) {
                            $res &= Configuration::updateValue('ANGARSLIDER_PAUSE', (int)Tools::getValue('ANGARSLIDER_PAUSE'), false, $shop_group_id);
                            $res &= Configuration::updateValue('ANGARSLIDER_PAUSE_ON_HOVER', (int)Tools::getValue('ANGARSLIDER_PAUSE_ON_HOVER'), false, $shop_group_id);
                            $res &= Configuration::updateValue('ANGARSLIDER_PAGER', (int)Tools::getValue('ANGARSLIDER_PAGER'), false, $shop_group_id);
                        }
                    }
                    break;
    ...
    public function getSlides($active = null)
    {
        ...
        foreach ($slides as &$slide) {
            $slide['image_url'] = $this->context->link->getMediaLink(_MODULE_DIR_.'angarslider/views/img/images/'.$slide['image']);
        ...
        $fields_form = array(
            'form' => array(
                ...
                'input' => array(
                    array(
                        'type' => 'text',
                        'label' => $this->getTranslator()->trans('Pause', array(), 'Modules.Imageslider.Admin'),
                    ...
                    array(
                        ...
                        'name' => 'ANGARSLIDER_PAUSE_ON_HOVER',
                    ...
                    array(
                        'type' => 'switch',
                        'label' => $this->getTranslator()->trans('Pager', array(), 'Modules.Imageslider.Admin'),
                        'name' => 'ANGARSLIDER_PAGER',
                        'desc' => $this->getTranslator()->trans('Show slider pager.', array(), 'Modules.Imageslider.Admin'),
                        'values' => array(
                            array(
                                'id' => 'active_on',
                                'value' => 1,

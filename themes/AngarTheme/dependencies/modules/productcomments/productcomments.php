...
class ProductComments extends Module
{
    ...
    public function __construct()
    {
        ...
        $this->templateFile = 'module:productcomments/views/templates/hook/productcomments_reviews.tpl';
    }
    ...
    private function _setBaseUrl()
    {
        ...
        foreach (Tools::getValue('CommentsValue') as $k => $value) {
            if (!in_array($k, array(
                'deleteCriterion',
                'editCriterion'
            ))
    ...
    {
        $tabz[] = (new PrestaShop\PrestaShop\Core\Product\ProductExtraContent())->setTitle($this->l('Reviews'))->setContent($this->hookDisplayProductTabContent($params));
        return $tabz;
    }

    public function hookDisplayCommentsExtra($params)
    {
        ...
        return $this->display(__file__, 'views/templates/front/productcomments-extra.tpl');
    }
    ...
    public function hookDisplayProductTabContent($params)
    {
        ...
        return $this->display(__FILE__, 'views/templates/front/productcomments.tpl');
    }
...
class ProductcommentsUpdate extends productcomments
{
    public static function version($version)
    {
        ...
        if (Tools::strlen($version) == 3) {
            $version = (int)$version . "0";
        }
        if (Tools::strlen($version) == 2) {
            $version = (int)$version . "00";
        }
        if (Tools::strlen($version) == 1) {
            $version = (int)$version . "000";
        }
        if (Tools::strlen($version) == 0) {
    ...
    public static function verify($module, $key, $version)
    {
        if (ini_get("allow_url_fopen")) {
            if (function_exists("Tools::file_get_contents()")) {
                $actual_version = @Tools::file_get_contents('http://dev.mypresta.eu/update/get.php?module=' . $module . "&version=" . self::encrypt($version) . "&lic=$key&u=" . self::encrypt(_PS_BASE_URL_ . __PS_BASE_URI__));
            }
        }

...
class SagePayCwSuccessModuleFrontController extends ModuleFrontController
{
	public $ssl = true;
	...
	public function initContent()
	{
		...
		$dbTransaction = SagePayCw_Entity_Transaction::loadById(intval($transactionId));
		...
		$timeoutUrl = $link->getModuleLink('sagepaycw', 'timeout', array('cw_transaction_id' => $dbTransaction->getTransactionId()), true);

		$failedUrl = $link->getModuleLink('sagepaycw', 'error', array(
		...
		$cookieKey = 'alias_sagepaycw_' . $dbTransaction->getModuleId();
		...
		while (true) {
			...
			$dbTransaction = SagePayCw_Entity_Transaction::loadById(intval($transactionId), false);

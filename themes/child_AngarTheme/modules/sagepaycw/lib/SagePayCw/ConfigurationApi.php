...
class SagePayCw_ConfigurationApi {
	...
	public function getConfigurationValue($key, $languageId = null) {
		...
		if (strpos($rs, self::ASSET_IDENTIFIER) === 0) {
			...
			$rs = SagePayCw_Util::getAssetResolver()->resolveAssetStream($asset);
		}
	...
	private function getConfigurationKeyLegacy($key, $languageId = null) {
		$key = 'SAGEPAYCW_' . $this->moduleId . '_' . $key;
		if ($languageId !== null) {
			$key .= '_' . $languageId;
		}
		...
		foreach ($fields as $fieldId => $field) {
			if ($field['type'] == 'orderstatus') {
				...
				if ($value instanceof Customweb_Core_Stream_Input_File) {
					$field['desc'] .= '<br />' . SagePayCw::translate("Current: ") . $value->getFilePath();
				}
				$newFields[$fieldId] = $field;
				$newField = array(
					'type' => 'checkbox',
					...
					'values' => array(
						...
						'query' => array(
							'reset' => array(
								'val' => 'RESET',
								'name' => SagePayCw::translate("Reset '@label' to default", array('@label' => $field['label'])),
								'id' => 'RESET',
							),
						),
					),
	...
	public function l($string, $specific = false) {
		return SagePayCw::translate($string, $specific);
	}

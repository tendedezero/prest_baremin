...
/**
 * 
 * @Entity(tableName = 'sagepaycw_transactions')
...
class SagePayCw_Entity_Transaction extends Customweb_Payment_Entity_AbstractTransaction
{
	...
	protected function authorize(Customweb_Database_Entity_IManager $entityManager) {
		...
		if (empty($orderId)) {
			...
			$stateId = SagePayCw_OrderStatus::getPendingOrderStatusId();
			...
			$GLOBALS['sagepaycw_successful_transaction_object'] = $transactionObject;
			$employeeId = null;
			...
			$message = SagePayCw_Util::getOrderCreationMessage($employeeId);
			...
			$paymentMethod->validateOrder(
					...
					$message,
					SagePayCw_Util::extractMailVariables($transactionObject),
					null,
					false,
					$customer->secure_key
			);
			...
			if (count($messages) > 0 && method_exists('Mail', 'sendMailMessageWithoutHook')) {
				...
				foreach ($messages as $message) {
					SagePayCw_Util::attachTransactionToMailMessage($this->getTransactionObject(), $message);
	...
	public static function getGridQuery() {
		return 'SELECT * FROM ' . _DB_PREFIX_ . 'sagepaycw_transactions WHERE ${WHERE} ${ORDER_BY} ${LIMIT}';
	}
	...
	public static function loadById($id, $cache = true) {
		return SagePayCw_Util::getEntityManager()->fetch('SagePayCw_Entity_Transaction', $id, $cache);
	}
	...
	public static function getTransactionsByOrderId($orderId, $loadFromCache = true) {
		return SagePayCw_Util::getEntityManager()->searchByFilterName('SagePayCw_Entity_Transaction', 'loadByOrderId', array('>orderId' => $orderId), $loadFromCache);
	}
	...
	public static function getTransactionsByCartId($cartId, $loadFromCache = true) {
		return SagePayCw_Util::getEntityManager()->searchByFilterName('SagePayCw_Entity_Transaction', 'loadByCartId', array('>cartId' => $cartId), $loadFromCache);
	}
	...
	public static function getTransactionsByOriginalCartId($cartId, $loadFromCache = true) {
		return SagePayCw_Util::getEntityManager()->searchByFilterName('SagePayCw_Entity_Transaction', 'loadByOriginalCartId', array('>cartId' => $cartId), $loadFromCache);
	}
	...
	public static function getTransactionsByCartOrOrder($cartId, $orderId, $loadFromCache = true) {
		return SagePayCw_Util::getEntityManager()->searchByFilterName('SagePayCw_Entity_Transaction', 'loadByCartOrOrder', array('>cartId' => $cartId, '>orderId' => $orderId), $loadFromCache);
	}

 ...
 * @Bean
 */
class SagePayCw_ExternalCheckoutService extends Customweb_Payment_ExternalCheckout_AbstractCheckoutService {

	public function loadContext($contextId, $cache = true){
		return $this->getEntityManager()->fetch('SagePayCw_Entity_ExternalCheckoutContext', $contextId, $cache);
	}

	public function renderShippingMethodSelectionPane(Customweb_Payment_ExternalCheckout_IContext $context, $errorMessages){
		if (!($context instanceof SagePayCw_Entity_ExternalCheckoutContext)) {
			throw new Customweb_Core_Exception_CastException('SagePayCw_Entity_ExternalCheckoutContext');
		}
		...
		return SagePayCw_Util::getTemplateRenderer()->render($templateContext);
	}

	private function registerSmartyFunctions(){
		$smarty = SagePayCw_Util::getTemplateSmartyContainer()->getSmartyInstance();
	...
	private function getShippingMethodNameFromContext(SagePayCw_Entity_ExternalCheckoutContext $context){
	...
	public function authenticate(Customweb_Payment_ExternalCheckout_IContext $context, $emailAddress, $successUrl){
		...
		$externalCheckout = SagePayCw::getInstance()->getConfigurationValue('external_checkout_account_creation');
	...
	protected function createTransactionContextFromContext(Customweb_Payment_ExternalCheckout_IContext $context){
		...
		if (!($paymentMethod instanceof SagePayCw_IPaymentMethod)) {
			throw new Customweb_Core_Exception_CastException('SagePayCw_IPaymentMethod');
		}
		$orderContext = new SagePayCw_OrderContext(new Cart($context->getCartId()), 
				new SagePayCw_PaymentMethodWrapper($paymentMethod));
	...
	public function getPossiblePaymentMethods(Customweb_Payment_ExternalCheckout_IContext $context){
		...
		foreach (PaymentModule::getInstalledPaymentModules() as $module) {
			if (strpos($module['name'], 'sagepaycw_') === 0) {
	...
	public function validateReviewForm(Customweb_Payment_ExternalCheckout_IContext $context, Customweb_Core_Http_IRequest $request){
		...
		if ($sessionContext->cookie->cart_total_amount != $cart->getOrderTotal()) {
			throw new Exception(SagePayCw::translate("Cart content was modified."));
		}
		
		$parameters = $request->getParameters();
		if ((int) (Configuration::get('PS_CONDITIONS')) && (!isset($parameters['cgv']) || $parameters['cgv'] !== '1')) {
			throw new Exception(SagePayCw::translate("You need to accept the general terms and conditions."));
		}
	}
	...
	private function updateMessage($messageContent, Cart $cart){
		if (!empty($messageContent)) {
			if (!Validate::isMessage($messageContent)) {
				throw new Exception(SagePayCw::translate('Invalid message'));
			}

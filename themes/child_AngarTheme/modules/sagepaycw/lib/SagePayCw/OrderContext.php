...
class SagePayCw_OrderContext extends Customweb_Payment_Authorization_OrderContext_AbstractDeprecated implements Customweb_Payment_Authorization_IOrderContext
{
	private $cartId;
	private $customerId;
	private $carrierName;
	...
	public function __construct($cart, Customweb_Payment_Authorization_IPaymentMethod $paymentMethod) {
		...
		$key = SagePayCw_Util::getCheckoutCookieKey($paymentMethod);
		...
		$this->employeeId = SagePayCw_Util::getEmployeeIdFromCookie();
	}
	...
	private function buildLineItems(Cart $cart, $orderTotal) {
		return SagePayCw_Util::createLineItemsFromCart($cart, $orderTotal, $this->getPaymentMethod());
	}

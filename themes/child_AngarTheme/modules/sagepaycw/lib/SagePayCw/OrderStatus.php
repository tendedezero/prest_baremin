...
class SagePayCw_OrderStatus {
	
	const LEGACY_SETTING_KEY = 'PENDING_ORDER_STATUS_ID';
	
	const SETTING_KEY = 'SAGEPAYCW_PENDING_STATE';
	
	private static $pendingOrderState = null;
	
	public static function getPendingOrderStatusId() {
		$result = SagePayCw::getInstance()->getConfigurationValue(self::LEGACY_SETTING_KEY);
		if (!empty($result)) {
			return $result;
		}
		else {
	...
	public static function createPendingOrderState() {
		...
		$pendingOrderState->module_name = 'sagepaycw';
		
		foreach (Language::getLanguages() as $language) {
			$pendingOrderState->name[$language['id_lang']] = 'Awaiting Payment (Sage Pay)';
		}
		...
		SagePayCw_OrderStatus::setPendingOrderStatusId($pendingOrderState->id);

 ...
 *
 */
require_once dirname(dirname(dirname(__FILE__))) . '/sagepaycw.php';

require_once 'Customweb/Payment/Authorization/IPaymentMethod.php';

class SagePayCw_PaymentMethod extends PaymentModule implements Customweb_Payment_Authorization_IPaymentMethod {
	/**
	 *
	...
	public $paymentMethodDisplayName = 'Sage Pay';
...
require_once 'SagePayCw/SmartyProxy.php';
	...
	public function hookPayment($params){
		...
		$link = new Link();
		$confirmationLink = $link->getModuleLink('sagepaycw', 'payment', array(
			'id_module' => $this->id
		), true);
		...
		if (!$this->existsPaymentMethodConfigurationValue('PAYMENT_FORM') ||
			...
			try {
				$orderContext = $this->getOrderContext();
				$adapter = SagePayCw_Util::getShopAdapterByPaymentAdapter($this->getAuthorizationAdpater($orderContext));
	...
	public function hookDisplayPaymentEU($params){
		...
		$link = new Link();
		$redirectionUrl = $link->getModuleLink('sagepaycw', 'payment', array(
			'id_module' => $this->id
		), true);
		...
		try {
			if (!$this->existsPaymentMethodConfigurationValue('PAYMENT_FORM') ||
					 $this->getPaymentMethodConfigurationValue('PAYMENT_FORM') != 'separate_page') {
				$adapter = SagePayCw_Util::getShopAdapterByPaymentAdapter($this->getAuthorizationAdpater($this->getOrderContext()));
	...
	public function hookPaymentReturn($params){
		...
		$orderId = $order->id;
		$transaction = current(SagePayCw_Entity_Transaction::getTransactionsByOrderId($orderId));
		$this->smarty->assign(
				array(
		...
		if ($transaction->getTransactionObject() !== null && $transaction->getTransactionObject()->isAuthorized()) {
			...
			if(!empty($paymentInformation)) {
				$result .=  '<div class="sagepaycw-invoice-payment-information sagepaycw-payment-return-table" id="sagepaycw-invoice-payment-information">';
				$result .= '<h4>' . SagePayCw::translate("Payment Information").'</h4>';
	...
	protected function getFormFields(){
		$fields = array(
			...
			array(
				'name' => 'PAYMENT_FORM',
				'label' => SagePayCw::translate('PAYMENT_FORM_LABEL'),
				'desc' => SagePayCw::translate('PAYMENT_FORM_DESCRIPTION'),
				'type' => 'select',
				'options' => array(
					'query' => array(
						array(
							'id' => 'payment_list_page',
							'name' => SagePayCw::translate('SHOW_ON_PAYMENT_LIST_PAGE')
						),
						array(
							'id' => 'separate_page',
							'name' => SagePayCw::translate('SHOW_ON_SEPARATE_PAGE')
						)
					),
					'name' => 'name',
					'id' => 'id'

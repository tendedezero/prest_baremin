...
class SagePayCw_TransactionContext implements Customweb_Payment_Authorization_PaymentPage_ITransactionContext,
	...
	public function __construct(SagePayCw_Entity_Transaction $transaction, SagePayCw_OrderContext $orderContext, $aliasTransactionId = NULL){
		$this->transactionId = $transaction->getTransactionId();
		$this->orderContext = $orderContext;
		$this->orderId = $transaction->getOrderId();
		$this->orderIdUnique = $transaction->isPendingOrderCreated();

		if (SagePayCw_Util::isAliasManagerActive($this->orderContext)) {
			$this->aliasTransactionId = $aliasTransactionId;
		}

		if (isset(Context::getContext()->employee->id)) {
			$this->adminUrl = SagePayCw::getAdminUrl('AdminSagePayCwMoto', array(
				'id_cart' => Tools::getValue('id_cart', null)
			));
		}

		$link = new Link();
		$this->failedUrl = $link->getModuleLink('sagepaycw', 'error', array(), true);
		$this->successUrl = $link->getModuleLink('sagepaycw', 'success', array(), true);
		$this->breakoutUrl = $link->getModuleLink('sagepaycw', 'breakout', array(), true);
	}
	...
	public function getInternalTransaction(){
		return SagePayCw_Entity_Transaction::loadById($this->getInternalTransactionId());
	}
	...
	public function getAlias(){
		...
		if ($this->aliasTransactionId !== null) {
			$transcation = SagePayCw_Entity_Transaction::loadById((int) $this->aliasTransactionId);
	...
	public function getPaymentCustomerContext(){
		return SagePayCw_Util::getPaymentCustomerContext($this->orderContext->getCustomerId());
	}

	public function getNotificationUrl(){
		$link = new Link();
		return $link->getModuleLink('sagepaycw', 'notification', array(), true);
	}

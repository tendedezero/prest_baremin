    ...
    <table class="supercheckout-summary">
        ...
        <tbody>
            ...
            {foreach $products as $product}
                    ...
                    <td style="display:{if $logged}{if $settings['cart_options']['product_name']['logged']['display'] eq 1}{else}none{/if}{else}{if $settings['cart_options']['product_name']['guest']['display'] eq 1}{else}none{/if}{/if};" class="supercheckout-name">
                        <div>
                            ...
                            {if $image_display eq 1}
                                <img width='{$settings['cart_image_size']['width']}' height='{$settings['cart_image_size']['height']}' src="{$product.cover.bySize.cart_default.url|escape:'quotes'}" alt='{$product.name}' onclick="showEnlargedImage(this)"/>
                                <br>
                                    ...
                                    {foreach from=$customization.fields item="field"}
                                        <small>
                                            ...
                                            {elseif $field.type == 'image'}
                                                <img src="{$field.image.small.url}" onclick="showEnlargedImage(this)">
                                            {/if}    
                                        </small>
                                    {/foreach}
                                {/foreach}
                ...
                {*start-MK made changes to display availability column*}
                {if $PS_STOCK_MANAGEMENT}
                <td style="display:{if $logged}{if $settings['cart_options']['product_availability']['logged']['display'] eq 1}{else}none{/if}{else}{if $settings['cart_options']['product_availability']['guest']['display'] eq 1}{else}none{/if}{/if};" class="supercheckout-model">
...
<table class="supercheckout-totals">
    ...
    <tbody>
        ...
        {if $vouchers.allowed}
            {foreach $vouchers.added as $voucher}
                <tr id="cart_discount_{$voucher.id_cart_rule}" class="cart_discount" style="display:{if $logged}{if $settings['order_total_option']['voucher']['logged']['display'] eq 1}{else}none{/if}{else}{if $settings['order_total_option']['voucher']['guest']['display'] eq 1}{else}none{/if}{/if};">
                    <td class="title"><b>{$voucher.name}<a href="javascript:void(0)" onclick="removeDiscount('{$voucher.id_cart_rule|intval}')"><div title="{l s='Redeem' mod='supercheckout'}" class="removeProduct"></div></a></td></b></td>
        ...
        <tr style="display:{if $logged}{if $settings['order_total_option']['total']['logged']['display'] eq 1}{else}none{/if}{else}{if $settings['order_total_option']['total']['guest']['display'] eq 1}{else}none{/if}{/if};">
            ...
            <td class="value"><span id="total_price" class="price">{$totals.total.value nofilter}{*escape not required as contains html*}</span>
